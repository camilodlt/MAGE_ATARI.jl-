...
mutable struct AtariEnv <: AbstractEnvironment
    ...
    function AtariEnv(romfile::AbstractString, seed::Int)
        # if !isfile(romfile)
        #     oldromfile = romfile
        ...
        new(ale, 0, false, 0.0, 0.0, 0, w, h, rawscreen, state, screen)
    end

    function AtariEnv(romfile::AbstractString, seed::Int, lck)
        ale = ALE.ALE_new()
        setInt(ale, "random_seed", Cint(seed))
        setFloat(ale, "repeat_action_probability", Float32(0.0))
        lock(lck) do
